...
def test_dispatch_request(db):
    ...
    DRF APIView render, parser, permissions and other attributes should
    be respected.
    """
...
def test_dispatch_request_model_view_set(db, basename):
    """List, retrieve, create, update & delete actions in the `ModelViewSet`
    defined with `Injector` subclass."""
...
def test_model_view_set_undefined_method(db):
    """Throw error if corresponding model view set action method is not defined
    in the injector."""
