...
def _make_callable_spec(func, funcname, owner_message):
    args = []
    varargs = kwargs = None
    for name, param in signature(func).parameters.items():
        ...
        if have_default:
            _check_argument_default(name, param.default, owner_message)
...
def _make_func_spec(func, funcname, owner_message):
    args = _make_callable_spec(func, funcname, owner_message)
    return dict(args)


def _make_method_spec(func, funcname, owner_message):
    args = _make_callable_spec(func, funcname, owner_message)
    return dict(args[1:])
